...
import static info.LoginInfo.getRand;
import static info.LoginInfo.parseXML;
...
//            retry...
    ...
    private static void redictforStatus(String url) throws IOException {
        HttpResponse response = httpClient.doGet(url, false);
        String redict = "";
    ...
    private static void getContact() throws IOException {
        if (FileUtils.existsOrCreateDirs(LoginInfo.contact_imgs)) {
            ...
            LoginInfo.friendsList = contactInfo.getMemberList();
            //下载好友头像
            /*for (User user : friendsList) {
                String urlStr = user.getHeadImgUrl();
                HttpResponse resp = httpClient.doGet(user.getHeadImgUrl(), false);
    ...
    public void logout() {
        ...
        HttpResponse response = httpClient.doPost(url, nameValuePairList, null, null);
    ...
    private void webwxinit() throws IOException {
        ...
        HttpResponse response = httpClient.doPost(url, null, param, null);
